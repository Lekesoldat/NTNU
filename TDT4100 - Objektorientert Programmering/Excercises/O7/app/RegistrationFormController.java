...
public class RegistrationFormController extends ScreenController<EntryManager> {
  @FXML
  ...
  protected void handleSubmitButtonAction(ActionEvent evt) {
    ...
    data.addEntry(new SleepEntry(dateStart.getValue(), timeStart.getText(), dateEnd.getValue(), timeEnd.getText()));
  }

  @FXML
  protected void handleShowEntriesButtonAction(ActionEvent evt) {
    List<SleepEntry> entries = data.getEntries();
  ...
  @FXML
  protected void handleSettingsButtonAction(ActionEvent evt) {
    screenManager.navigate("settings_panel", data);
  }
}
